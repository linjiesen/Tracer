...
def payment(request, policy_id):
    ...
    if request.tracer.price_policy.category == 2:
        # 找到之前的订单, 总支付费用-（开始~结束时间）*剩余天数=抵扣的钱
        _object = models.Transaction.objects.filter(user=request.tracer.user, status=2).order_by('-id').first()
        total_timedelta = _object.end_datetime - _object.start_datetime  # 剩余天数
...
def pay(request):
    ...
    private_key = RSA.importKey(open("files/私钥.txt").read())
...
def pay(request):
    ...
    query_params = ali_pay.direct_pay(
        ...
        total_amount=total_price,  # 交易金额(元，保留两位小数)
    )
